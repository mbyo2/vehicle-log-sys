...
-- 2. Fix overly permissive profile access - restrict cross-user data access
DROP POLICY IF EXISTS "Company admins can view company profiles (restricted)" ON public.profiles;

CREATE POLICY "Restricted company admin profile access"
ON public.profiles
FOR SELECT
USING (
  auth.uid() = id OR 
  EXISTS (
    ...
    AND (
      ...
      (p.role = 'company_admin'::user_role AND p.company_id = profiles.company_id)
    )
  )
);

-- 3. Add audit logging trigger for MFA access
...
-- 4. Add audit logging for profile data access
...
CREATE TRIGGER erp_integration_access_audit
  AFTER SELECT OR UPDATE ON public.erp_integrations
  FOR EACH ROW EXECUTE FUNCTION log_integration_access();

CREATE TRIGGER external_integration_access_audit
  AFTER SELECT OR UPDATE ON public.external_integrations
  FOR EACH ROW EXECUTE FUNCTION log_integration_access();
